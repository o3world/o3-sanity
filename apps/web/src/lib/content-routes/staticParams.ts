 ...
 * `generateStaticParams` that returns nothing, so a swallowed failure would
 * surface as a build error one step removed from its cause. Letting the fetch
 * throw puts the message — which names the missing token — where the failure
 * is.
 */
