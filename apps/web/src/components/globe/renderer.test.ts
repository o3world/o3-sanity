...
it('updates the footer glow before measuring the globe for each GPU frame and releases it on abort', async () => {
  ...
  const globe = {
    closest: (selector: string) => (selector === '.cta-lag' ? glow : hero),
  ...
  vi.stubGlobal('document', { ...listeners, hidden: false, documentElement: { clientHeight: 844 } })
  ...
  try {
    ...
    tick(0)
    expect(measured[0]).toBe('0 -50px')
    expect(values.get('animation')).toBe('none')
    top = -500
    tick(2000)
    const position = Number(measured[1]?.split(' ')[1]?.replace('px', ''))
    expect(position).toBeGreaterThan(-50)
    expect(position).toBeLessThan(-40)
    controller.abort()
