...
- Draft-only client components load through a client-side `next/dynamic` boundary — `VisualEditing` and `ClientBlockRenderer` are those boundaries (#269). Turbopack gives a route one eager chunk group and puts every client entry of that route in it, so a `'use client'` module a server component imports is downloaded by every visitor whether or not it renders. `{isDraft ? … }` gates the render, not the download, and `next/dynamic` called from a server component still registers a client entry. `pnpm build:assert` is what catches a regression.
