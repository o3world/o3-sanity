 ...
 *
 * The root is a block, one of its array members (#122), or a shared object
 * (#145) — the generated field is the same at all three, because every knob
 * path is relative to the root it is declared against.
 *
 ...
 *
 * ONE SPLICE FOR EVERY ROOT. An array member's and a shared object's fields are
 * generated by the same rule for the same reason: get it wrong there and the
 * type's field order moves, which moves `generated.ts` under the renderer.
 * `where` names the factory in the error so an author is told which call to
 * fix, not which helper.
 */
