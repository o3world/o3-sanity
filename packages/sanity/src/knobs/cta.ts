...
/**
 * The call to action's one design option (#145, ADR 0023) — the last of #113's
 * closed sets to leave the schema and become a declaration.
 *
 * A `cta` sits on eight blocks, inside three array members, in a layout column
 * and in the nav and footer. That is the argument for the root in one line:
 * the fill an editor picks is the same choice wherever the button stands, so it
 * is declared against the component and read at every placement.
 *
 * Menu-only, like every instance knob — `barKnobs` never sees an object spec.
 *
 * `label`, `target` and `href` stay hand-written in `cta.ts`. `href`'s gate
 ...
 */
export const ctaKnobs = defineObjectKnobs({
  type: 'cta',
