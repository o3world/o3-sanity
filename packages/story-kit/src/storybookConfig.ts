...
import { REPO_ROOT_PREFIX, SHARED_STORY_ROOTS, STORY_GLOB_SUFFIX } from './storyRoots'

export interface StorybookHostOptions {
  /** Absolute path of the host's own `.storybook` directory. */
  configDir: string
  /**
  ...
  /**
   * A captured-prototypes directory (ADR 0010), relative to `configDir`. The
   * host that has one globs it for stories and mounts the sets inside it.
   */
...
export function defineStorybookConfig({
  ...
  appStoryRoots = [],
  prototypesDir,
}: StorybookHostOptions): StorybookConfig {
  const roots = [...SHARED_STORY_ROOTS, ...appStoryRoots]
  const stories = roots.map((root) => `${REPO_ROOT_PREFIX}/${root}/${STORY_GLOB_SUFFIX}`)
  if (prototypesDir) stories.push(`${prototypesDir}/${STORY_GLOB_SUFFIX}`)

  return {
    stories,
    staticDirs: prototypeMounts(configDir, prototypesDir),
    addons: [
    ...
    viteFinal: async (cfg: InlineConfig) => viteFinal(cfg, configDir),
  }
}
 ...
 * to forget.
 */
function prototypeMounts(configDir: string, prototypesDir: string | undefined) {
  if (!prototypesDir) return []
  const absolute = path.resolve(configDir, prototypesDir)
  ...
  return fs
    ...
    .filter(
      (entry) =>
        entry.isDirectory() && fs.existsSync(path.join(absolute, entry.name, 'index.html')),
    )
    .map((entry) => ({
      from: `${prototypesDir}/${entry.name}`,
      to: `/prototypes/${entry.name}`,
    }))
}

function viteFinal(cfg: InlineConfig, configDir: string): InlineConfig {
  ...
  // (packages/ui, apps/web) re-import React directly. Two React copies →
  ...
  // every React entry to the copy the HOST resolves, so there's exactly one
  // instance across the bundle.
  const require_ = createRequire(path.join(configDir, 'main.ts'))
  const reactEntries = {
  ...
  // Replace the addon's alias set: pin every React entry to the host's copy
  // (see reactEntries above). Anchored regexes = exact match for the bare
  // import paths only.
  ...
  // the host's copy. Excluding each offending package from pre-bundling
  ...
  // primitives, apps/web chrome). Vite strips the directive when bundling
  // for Storybook (non-Next) and emits a MODULE_LEVEL_DIRECTIVE warning per
  // module, plus a follow-on SOURCEMAP_ERROR ("Can't resolve original
  // location of error") because stripping the line shifts the source map.
  // The flood is harmless but intermittently tips `storybook build` to a
  // non-zero exit. Drop source maps for the Storybook bundle (it's a static
  // doc build, not debugged) and silence just those two warning codes so the
  // build is deterministic — every other warning still surfaces.
