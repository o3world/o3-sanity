...
4. `packages/sanity/src/schemas/index.ts` — import and add to the keyed `SECTION_SCHEMAS` record; `schemaTypes` loads every block the roster (step 1) names.
...
6. `packages/content-ui/src/blocks/section/<name>Section/<Name>Section.tsx` — folder name === schema name exactly. Type props with `SectionProps<'<name>Section'>` from `@o3/content-runtime/blocks`; never hand-write the prop shape. That is the default home; a renderer goes in `apps/web/src/content/` only when it needs something only the app has — see `AGENTS.md` → Where a component lives.
7. Export it from `packages/content-ui/src/index.ts`, then add a `defineBlockRender('<name>Section', { component: … })` entry to `CLIENT_SECTION_BINDINGS` in `apps/web/src/content/blocks/clientComponents.tsx`.
...
3. Routable? Add to `ROUTABLE_TYPES` in `constants.ts`; a collection also needs its prefix and title in `collections` in `brand.ts`. Then one folder under `apps/web/src/content/documents/<type>/` with `entry.tsx` + registry line; its prefix comes from `collectionPrefixes()`, never a literal.
...
A rename is a content migration: editors author in `production`, so the dataset change ships as a targeted migration under `tools/migration/src/migrations/` (`AGENTS.md` → Changing a dataset), alongside the code and any committed JSON under `tools/migration/data/`.

Rename in one commit, all of it: registry → schema → `index.ts` → `queries.ts` arms → renderer folder **and** file **and** component name → `clientComponents.tsx` binding → any `Extract<…, { _type: '…' }>` references → committed JSON under `tools/migration/data/` → `docs/specs/schema.md` → the CONTEXT.md lexicon or known-drift list. Then `pnpm typegen && pnpm check`.
