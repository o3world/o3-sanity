...
6. `packages/content-ui/src/blocks/section/<name>Section/<Name>Section.tsx` — folder name === schema name exactly. Type props with `SectionProps<'<name>Section'>` from `@o3/content-runtime/blocks`; never hand-write the prop shape. That is the home for a block both brands draw the same way; when the two designs diverge the renderer belongs in each app instead — see the placement table in `AGENTS.md` → Where a component lives.
