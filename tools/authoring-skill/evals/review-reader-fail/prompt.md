...
`drafts.brief-eval-review-reader-fail-zero-violations`.

```json
{
  "_id": "brief-eval-review-reader-fail-zero-violations",
  "_type": "brief",
  "key": "eval-review-reader-fail-zero-violations",
  "title": "Automated accessibility audits and what they miss",
  "stage": "draft",
  "nextStep": "Run the review gates on the draft and record a verdict.",
  "background": "From Priya (engagement lead), in conversation: we audited the client's design system in March. axe-core ran against all 42 templates in the library and returned 0 violations. The team had been fixing axe findings for a year using the same tool. An automated checker rules only on what is decidable from markup alone — a missing alt attribute, a contrast ratio under the threshold, a form control with no label. Our own manual pass on the same 42 templates found 11 that could not be operated from the keyboard: a modal that trapped focus outside itself, a carousel whose controls were unlabelled divs, and a filter panel that reordered results silently. The client had been reporting the zero to their board as an accessibility score.",
  ...
  "readerQuestions": [
    ...
    "Which kinds of defect does an automated checker actually decide, and which can it not decide at all?",
    "What should a team do instead of reporting the scan result?"
  ],
  "outline": "1. Open on the clean result and the first Tab press. 2. What a scan can decide, and what it cannot. 3. What the keyboard found. 4. The turn: the number measured the tool. 5. Ending.",
  "draft": {
...
a question, Priya says "I don't have that" and it stays open. Anywhere a nod is
wanted, Priya nods to your recommendation.
...
`dataset/brief-eval-review-reader-fail-zero-violations.json` in your working
