...
back is `drafts.brief-eval-review-verdict-zero-violations`.

```json
{
  "_id": "brief-eval-review-verdict-zero-violations",
  "_type": "brief",
  "key": "eval-review-verdict-zero-violations",
  "title": "Automated accessibility audits and what they miss",
  "stage": "draft",
  "nextStep": "Run the review gates on the draft and record a verdict.",
  "background": "From Priya (engagement lead), in conversation: we audited the client's design system in March. axe-core ran against all 42 templates in the library and returned 0 violations. The team had been fixing axe findings for a year using the same tool. Our own manual pass on the same 42 templates found 11 that could not be operated from the keyboard: a modal that trapped focus outside itself, a carousel whose controls were unlabelled divs, and a filter panel that reordered results silently. The client had been reporting the zero to their board as an accessibility score.",
...
- `dataset/brief-eval-review-verdict-zero-violations.json` — the brief read
  back out of the dataset, verbatim, after every write you made. Read the draft
  id. A claim about the brief the dataset does not confirm is a claim about
  nothing.
- `reader-prompt.md` — if the run hands a document to a context-free reader,
  the exact text that reader was given, verbatim. If it hands the reader
  nothing, write the file saying so.
