...
// FORK (cfg.libOverrides): this repo's stories pin one of three O3 surfaces
// with `globals: { backgrounds: { value: 'ink' | 'bone' } }` — a Storybook
// backgrounds-addon global the upstream wrapper drops (`globals: {}`), so a
// light-on-ink component previewed on white reads as unstyled or invisible.
// 20 of 29 components do this. The wrapper now carries the story's globals
// and paints the surface, and honours `parameters.layout` the way the
// storybook canvas does. Surface values come from
// apps/storybook/.storybook/preview.ts (backgrounds.options).
...
const COMPOSE = `function compose(S: any, key: string) {
  ...
  const g: any = { ...(meta.globals ?? {}), ...((st && st.globals) ?? {}) };
  const ctx: any = {
    args, name: key, title, kind: title, id: '', componentId: '',
    globals: g, viewMode: 'story',
    // Storybook MERGES parameters (meta <- story); the stock wrapper replaced
    // them, so a story declaring any parameter of its own (a design link,
    // controls) silently dropped meta's fullscreen layout and got padded.
    parameters: { ...(meta.parameters ?? {}), ...((st && st.parameters) ?? {}) },
  };
  ...
  // The three O3 surfaces, from .storybook/preview.ts backgrounds.options.
  const SURFACES: any = { white: '#ffffff', bone: '#efeeec', ink: '#030303' };
  const bg: string | undefined = SURFACES[String(g?.backgrounds?.value ?? '')];
  if (!bg) return composed;
  // Mirror the storybook canvas's own layout semantics.
  const layout: string = String(ctx.parameters?.layout ?? 'padded');
  const style: any = { background: bg, boxSizing: 'border-box', minHeight: '100%' };
  if (layout === 'centered') Object.assign(style, { display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' });
  else if (layout !== 'fullscreen') style.padding = '16px';
  return () => React.createElement('div', { style }, composed());
}`
