...
| `/about`                                  | `1924:5344`               | seed-from-frame       | `page-seed-about` — transcribed; the feature grid, team and Careers bands render through their own blocks (#56), and the frame's band imagery is committed under `seed/assets/`. Careers is a section, not a route                                                                                                                                                                                                                                                                                                                                                                      | #46 ✅   |
| `/solutions`                              | `2360:2879` — **no 402**  | seed-from-frame       | `page-seed-solutions` — rebuilt against the 2026-08 redesign (#93): Interior Hero, an Overview `layoutSection` (photo committed under `seed/assets/`), the service grid as `railPanelsSection` `layout: grid`, the proof-point band as `layoutSection` with the molecule `decoration`, use cases as `featureGridSection` `layout: rows`. Provisional: the frame authors one use case ×3, and its foil/typo copy is amended — the note on the seed itemises it. The 24 WordPress services consolidate **into this page** — [ADR 0013](./adr/0013-services-consolidate-into-solutions.md) | #93      |
...
Three routes have a canonical desktop frame and **no 402 counterpart**: `/about`
(`1924:5344`), `/solutions` (`2360:2879`) and `/partners/sanity` (`2354:2446`).
