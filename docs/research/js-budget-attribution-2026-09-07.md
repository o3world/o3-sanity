...
Rebased onto `eefb1979` after the separate logo-wall loading/motion update landed. The combined production build measures **679,026 bytes** per content route, still below 680,000, with **50,974 bytes** under the absolute ceiling. `pnpm verify` passed all 47 tasks; the full suite passed 3,712 tests in 309 files. Rendering and bundle assertions passed, and a headed-browser check confirmed the solid CTA over the active starfield and working carousel navigation. The earlier attribution table remains pinned to `7281a4d2` so the isolated experiments remain comparable.
