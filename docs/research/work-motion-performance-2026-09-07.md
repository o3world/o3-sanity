...
## Default site integration

The accepted cards and work sky now live in `apps/web/src/components/work/` and render through the normal case showcase binding. The work build flag, query switches, and startup selector are removed. The shared globe production gate is also removed: every environment uses the same provider, homepage startup, navigation utilities, and starfield. Case-study chapters, media captures, and screen grids use the accepted entrances across case studies instead of only IRONMAN. Reduced-motion preferences, offscreen pausing, deferred GPU imports, and SVG recovery remain in place.

Validation after promotion: 308 test files and 3,695 tests passed, including non-IRONMAN chapter and screen-grid wiring. Web lint and the default production build passed. Rendering strategy, cached-404 behavior, and route budgets passed; content routes load 696,378 bytes against the 730,000-byte budget. Browser inspection at `http://localhost:3612/` without query parameters confirmed three organic cards, the work sky, and the GPU hero. Vertex rendered two chapter heading/lead sequences and two screen entrances; browser Back restored the card position with the sky visible and no horizontal overflow. These checks do not replace the physical-device GPU and compositor measurements listed above.

Use `pnpm --filter @o3/web preview` for the optimized local build on port 3612. Earlier flagged commands and prototype paths in this report describe the historical measurements, not the current startup procedure.
