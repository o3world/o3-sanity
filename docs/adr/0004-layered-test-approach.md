...
- **The suite is a checkpoint, not a loop.** No lefthook hook; `pnpm verify` is unchanged. CI runs it as its own job — `--changed` against the base on PRs, everything on pushes to main. Agents run `pnpm test` at milestones (before opening a PR, after a migration batch), not after every edit.
