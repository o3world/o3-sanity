...
- **Page** — a modular marketing page composed of a two-tier **sections** array (see ADR 0001). Carries `pageType`, a closed developer-managed enum: `standard | service | partner`. Service pages get a conditional **card** fieldset (short title, excerpt, icon) that listing blocks project — but **no service page exists or is planned** (ADR 0013; see Known drift). **Partner** is a platform-partnership landing page under `/partners/{slug}` — `/partners/sanity` is the first (#92), with Vercel and Lovable behind it; it is the first `pageType` value with both a document and a canonical frame. Slugs are multi-segment and carry their prefix (`ventures/rec-philly`). Ventures are ordinary standard pages — deliberately not a type.
